...
export function RequestForm() {
  ...
  if (success) {
    return (
      <div className="streamdock-card p-8 text-center">
    ...
    <form onSubmit={onSubmit} className="space-y-5">
      <Field icon={<Link2 size={14} />} label="Site URL">
        <input
          ...
          className="streamdock-card w-full bg-[var(--bg-elev)] px-4 py-2.5 text-sm outline-none focus:border-[var(--border-strong)]"
        />
      </Field>
      ...
      <div>
          ...
          {targets.map((row) => (
            <div key={row.id} className="flex flex-col gap-2 sm:flex-row">
              <select
                ...
                className="streamdock-card min-w-0 flex-1 bg-[var(--bg-elev)] px-3 py-2.5 text-sm outline-none focus:border-[var(--border-strong)]"
              >
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="streamdock-pill h-auto shrink-0 px-3 disabled:opacity-30"
                >
                  <X size={14} />
                </button>
              </div>
        ...
        <button
          type="button"
          onClick={addTarget}
          className="streamdock-pill mt-3 inline-flex items-center gap-2 px-4 py-2 text-sm"
        >
      ...
      <Field icon={<Star size={14} />} label="Why should we add it?">
        <textarea
          ...
          className="streamdock-card w-full resize-y bg-[var(--bg-elev)] px-4 py-2.5 text-sm outline-none focus:border-[var(--border-strong)]"
        />
      </Field>
