...
const KEY = "streamdock-recents-v1";
const MAX = 8;
...
export function addRecent(item: Omit<Recent, "visitedAt">) {
  ...
  try {
    ...
    window.dispatchEvent(new CustomEvent("streamdock-recents-changed"));
  } catch {}
}
...
export function RecentlyVisited() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("streamdock-recents-changed", load);
    return () => {
      cancelled = true;
      window.removeEventListener("streamdock-recents-changed", load);
    };
  }, []);
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {items.map((r) => (
          <a
            ...
            className="streamdock-card flex shrink-0 items-center gap-2 px-3 py-2 text-sm"
          >
            <Image
